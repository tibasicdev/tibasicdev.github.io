...
TI-68k calculators support a built-in way to add comments. When you type the copyright character [©](68k:comment.html) in a program line or expression, the rest of the line will be considered a comment, not code.
...
This is a built-in way to indent code, and in fact the number of indentations is stored automatically with each line of code. This means that (unlike most other formatting) the indentations won't disappear after the program is [tokenized](68k:tokenization.html), and also don't increase the program size. 
...
When choosing variable names, you might want to add a way to indicate the variable type, since it's otherwise hard to tell in some cases. Except in rare cases such as [68k:For](68k:for.html) loop indices, avoid using the same variable for different purposes. Finally, it's a good idea to add a comment at the beginning of the program explaining the purpose of the variables.

<center>

|**[<< Planning Programs](68k:planning.html)**|**[Overview](68k:development-overview.html)**|**[Debugging Programs >>](68k:debugging.html)**|
| --- | --- | --- |
