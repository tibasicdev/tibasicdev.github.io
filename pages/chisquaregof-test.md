...
(Note that in this case, if you suspected the number of customers to be higher on weekends, you could use a more sensitive test for only two categories: [`2-SampTTest`](2-sampttest.html))

## Advanced Uses

The `χ²GOF-Test(` command is only on TI-84 Plus and newer calculator models. However, it's possible to use the [`χ²cdf(`](chisquarecdf.html) command to simulate it on the other calculators: see the [χ² Goodness-of-fit Test](goodness-of-fit.html) routine.
## Formulas
...
- **[ERR:DIM MISMATCH](errors.html#dimmismatch)** is thrown if the two lists are of different length.
- **[ERR:DOMAIN](errors.html#domain)** is thrown if they only have one element, or if *df* is not a positive integer.
...
- [χ² Goodness-of-fit Test](goodness-of-fit.html)
