...
In TI-Basic, the first kind of loop is created with the [While](while.html) and [Repeat](repeat.html) commands, and the second kind is created with the [For(](for.html) command.
...
The [For(](for.html) loop is probably the most complicated command we've covered so far. Its syntax:
```
...
These methods are discussed in more depth in the [Validation](validation.html) tutorial.
...
Later in this tutorial, you'll learn about the [rand](rand.html) command, which can be used as an alternative to For( in creating delays.

<center>

|**[<< Labels](sk:labels.html)**|**[Table of Contents](starter-kit.html)**|**[Ans >>](sk:ans.html)**|
| --- | --- | --- |
