...
The first check is if N=G: if the guess is correct. If it is, we go to the Then part of the first If. Here we display "You've got it!", and how many guesses it took. We pause the program with [Pause](pause.html), so that the user can read the info we'll provide. Then with the [Stop](stop.html) command, we exit the program.
...
|**[<< Chapter Exercises](sk:exercises-basics.html)**|**[Table of Contents](starter-kit.html)**|**[More on the Home Screen >>](sk:homescreen.html)**|
| --- | --- | --- |
