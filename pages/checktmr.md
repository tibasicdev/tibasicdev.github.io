...
Making (number) higher increases accuracy, but takes longer. Also, make sure not to modify the variables `*n*` or `T` inside the [`For(`](for.html) loop.
...
When testing code, be aware that many different things affect the time: the strength of the batteries, the amount of free RAM, and including the closing parenthesis on the For( loop. The last one, in particular, has an impact when using a single-line [`If`](if.html) statement or one of the [`IS>(`](is-.html) and [`DS<(`](ds-.html) commands on the first line inside a For( loop.
