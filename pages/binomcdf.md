...
The [`binompdf(`](binompdf.html) and `binomcdf(` commands are the only ones apart from [`seq(`](seq-list.html) that can return a list of a given length, and they do it much more quickly. It therefore makes sense, in some situations, to use these commands as substitutes for [`seq(`](seq-list.html). 
...
Since "at most N" is equivalent to "0 or 1 or 2 or 3 or ... N", and since we can combine these probabilities by adding them, we can come up with an expression for `binomcdf(` by adding up values of [`binompdf(`](binompdf.html):

$$
\operatorname{binomcdf}(n,p,k) = \sum_{i=0}^{k}\operatorname{binompdf}(n,p,i) = \sum_{i=0}^{k}\binom{n}{i}\,p^i\,(1-p)^{n-i}$$

(If you're not familiar with sigma notation, $\sum_{i=0}^{k}$ just means "add the following up for each value of i 0 through k")

## Error Conditions

- **[ERR:DATATYPE](errors.html#datatype)** is thrown if you try to generate a list of probabilities with *p* equal to 0 or 1, and at least 257 trials.
