...
Below you can find the byte values for all the 83 series tokens, organized by category and two-byte prefix. For up-to-date and machine-readable tables, consider using the [TI Toolkit token sheets](https://github.com/ti-toolkit/tokens).

- [One-byte tokens](one-byte-tokens.html)
- [Two-byte user variable tokens](variable-tokens.html)
- [Two-byte statistical variable tokens](statistics-tokens.html)
- [Two-byte window and finance variable tokens](window-tokens.html)
- [Two-byte graph format tokens](format-tokens.html)
- [Miscellaneous two-byte tokens](miscellaneous-tokens.html)
- [TI-84+(C(S)E) only two-byte tokens](time-tokens.html)

There are several ways to insert a token given its hex value; all of them require an assembly program of some form. One of the easiest is to create an assembly program using [AsmPrgm](asmprgm.html) followed by the hex codes you want to convert to tokens, assemble it using [AsmComp(](asmcomp.html), and then unlock it (there are many programs that allow you to do this).
