# Xsnake
![Xsnake](xsnake/xsnake.gif style="width:200px; height:135px;" "Xsnake")

|Program Summary|Program Size|Uses Libraries?|Calculator Compatibility|Author|Download|
| --- | --- | --- | --- | --- | --- |
|A quality version of the classic snake game.|1,468 bytes|No, it's pure TI-Basic|TI-83/84/+/SE|[XprO](http://www.ticalc.org/archives/files/authors/35/3560.html)|[xsnake.zip](http://tibasicdev.github.io/local--files/xsnake/xsnake.zip)|
...
The main innovation that allowed Xsnake to be so fast is that the four arrow directions are each split up into their own separate loop, which cuts down on [If](if.html) conditionals and other speed impediments. The game also uses the [pxl-Test(](pxl-test.html) command for collision detection, which is faster than accessing a variable (like most other snake games do).
