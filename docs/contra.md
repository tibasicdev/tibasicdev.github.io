# Contra
![Contra](contra/contra.gif "Contra")

|Program Summary|Program Size|Uses Libraries?|Calculator Compatibility|Author|Download|
| --- | --- | --- | --- | --- | --- |
|A remake of the classic contra game.|10,000 bytes|No, it's pure TI-Basic|TI-83/84/+/SE|[Bryan Thomas](http://www.unitedti.org/index.php?showuser-4)|[contra.zip](http://tibasicdev.github.io/local--files/contra/contra.zip)|
...
The way that Contra got around the issue of loading and drawing the graphics is actually fairly commonplace now, but it consisted of [compressing](compression.html) all of the lines that make up a level, storing the four coordinates of a line in one list element. You then store the lines in a list, and loop over the entire list to display the level.

Likewise with controlling the player and enemies, Contra placed those functions in a separate [subprogram](subprograms.html). Using the subprogram allowed the main program to run much faster, since the program is more compact and it doesn't need to check for those things every time through the main game loop.
