...
- **F** is the test statistic. If the null hypothesis is true, it should follow Snedecor's F distribution, and [Fcdf(](fcdf.html) can be used to determine the p-value.
...
- **[ERR:ARGUMENT](errors.html#argument)** is thrown if one of the lists is blank, only one list is used, or the function is completely blank.
- **[ERR:SYNTAX](errors.html#syntax)** is thrown if you do not use lists (Matrixes, numbers,etc)
- * **[ERR:INVALID DIM](errors.html#invalid dim)** is thrown if you use a list that has 0 or a negative number.
- * **[ERR:DATA TYPE](errors.html#data type)** is thrown by using "l" or a list with a different set of data.
