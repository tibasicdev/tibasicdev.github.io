...
When displaying a matrix or a list, and the matrix or list is too large to display in its entirety, an ellipsis (…) is displayed at the boundaries of the screen. The matrix or list, unfortunately, cannot be scrolled so the rest of it can be seen (use the [`Pause`](pause.html) command instead).
...
The result is that you can never display text on the last line of the screen using the `Disp` command; you need to use the [`Output(`](output.html) command. (Using [`Output(`](output.html) does not have any affect on `Disp` and its text.) Also, if you have more than seven lines of text to display, you will need to place the [`Pause`](pause.html) command after every seven lines to prevent the screen from scrolling. These two scenarios come up fairly often, so it is good to know how to deal with them.
