# Glossary
[#](#Num) | [A](#A) | [B](#B) | [C](#C) | [D](#D) | [E](#E) | [F](#F) | [G](#G) | [H](#H) | [I](#I) | [K](#K) | [L](#L) | [M](#M) | [N](#N) | [O](#O) | [P](#P) | [R](#R) | [S](#S) | [T](#T) | [U](#U) | [V](#V)
...
: **[Ans](ans.html)** : The last answer that was stored in the calculator. The calculator also keeps a history of the last few statements entered in on the home screen where general  usage of the calculator occurs.

: **Application (*App*)** : A specialized assembly program that is stored in [Flash](#flash) ROM and is run through the APPS menu. There are ten 16KB pages available for Flash Applications on the TI-83+, while there are ninety-four 16KB pages available for Flash Applications on the TI-83+SE.
...
: **Argument (*Arg*)** : One of the values that a [command](#command) acts on, or an expression that is supplied as input to a program, function, or command.
...
: **[Binary](binandhex.html)** : The two-digit ([bit](#bit)) [number system](#number-system) based on 0 and 1, similar to the way the normal [decimal](#decimal) system is based on the numbers 0-9. For example the number 101 in binary represents 1*2<sup>2</sup>+0*2+1, or 5 in decimal.
...
: **Breakpoint** : A point in a program where program execution can be suspended, used when [debugging](#debugging) the program. In [TI-Basic](#tibasic), such breakpoints can be created with the [Pause](pause.html) command.
...
: **Character (*Char*)** : A letter, number, punctuation symbol, or special graphics symbol. They are encoded in [ASCII](#ascii) format internally, but the [TI-Basic](#tibasic) interpreter uses [tokens](#token) instead.

: **Clock cycle** : The unit of time in a [CPU](#cpu). It is so small that even the most primitive [assembly](#assembly) instructions take around 10 clock cycles to run; the amount of clock cycles it takes for a single [TI-Basic](#tibasic) command can be in the millions.

: **Command (*Cmd*)** : A word or token that tells the calculator to do something. Examples: [ClrHome](clrhome.html), [Output(](output.html), [While](while.html), etc. They are sometimes subdivided into [functions](#function) and [instructions](#instruction)

: **Complex** : Any expression or number which has an imaginary component (see *[i](i.html)*). For example, 3 + 2i is a complex number, where *i*=sqrt(−1).

: **Concatenation** : Joining two or more [strings](#string) together to make a longer string. The "+" is the concatenation operator. For example, "TI-"+"BASIC"="TI-BASIC".
...
: **Debugging (*Debug*)** : Fixing a [program](#program) to remove [bugs](#bug), or mistakes. There are many [techniques](debug.html) for debugging, some universal, some specific to calculator programming.

: **Decimal (*Dec*)** : A base 10 [number system](#number-system) using the symbols 0-9. It is the only number system natively supported by the calculator.

: **Default** : A standard characteristic or value which the calculator assumes without specification. For example, the [round(](round.html) command has a default of rounding 9 digits past the first digit, so round(X) will round X to 9 digits. This can be overwritten with a second [argument](#argument) to specify the digits to round to — round(X,3), for example, rounds to 3 digits.
...
: **Equation Operating System (*EOS*)** : The Equation Operating System (EOS<sup>TM</sup>) is the routine in the calculator that determines order of operations.
...
: **eZ80**: The Zilog eZ80 is the chip used in the TI-84+CE. If an [](# assembly) program is said to be in eZ80, then it will only work on the CE, an eZ80 calculator.
...
: **[Friendly Window](friendly-window.html)** : A configuration of window variables that's most useful for your program — most commonly, because it makes the coordinate value of a pixel come out to a round value, such as an integer, or .1 of an integer.

: **Function (*Func*)** : Although the word has many meanings, in the context of TI-83 series calculator programming, it's a type of [command](#command) that returns a value — such as [gcd(](gcd.html) or [sin(](sin.html).
...
: **[Hexadecimal](binandhex.html)** : A base 16 [number system](#number-system) using 16 symbols, 0-9 and A-F. It is used as a convenient shorthand way to express [binary](#binary) code, because every four bits of binary have a corresponding hexadecimal symbol.
...
: **Instruction** : In the case of TI-83 series programming, this is often used to indicate a type of [command](#command) that does not return a value, but stands alone: for example, [Line(](line.html), or [Input](input.html). It is also sometimes used to describe commands in general.
...
: **[List](lists.html)** : A sequence of numerical [variables](#variable) which can be accessed and modified by their position.
...
: **[Matrix](matrices.html) (*Matr*)** : A two-dimensional grid of numerical [variables](#variable) which can be accessed and modified by their position.
...
: **[Memory Leak](memory-leaks.html)** : A type of [bug](#bug) in which the program wastefully uses memory, often culminating with an error. In TI-Basic, this type of bug is often due to exiting a [loop](while.html) or [If](if.html)-Then-End block with a [Goto](goto.html).
...
: **[Prompt](prompt.html)** : A command that requests input from the user.

: **Pt** : On the monochrome calculators, a point is a specific 1x1 space on the calculator's graph screen. You can use the [Pt commands](pt-on.html) to manipulate a point. The primary difference between a pixel and a point is that a pixel is not affected by the graph screen settings, while a point is.
...
: **[Rand](rand.html)** : A command used to generate a  uniformly-distributed pseudo-random number between 0 and 1.

: **[Recursion](recursion.html)** : A program that calls itself as a subroutine (or a function that's defined in terms of itself). If not done carefully, this can result in a [](# memory-leak) See [recursion](#recursion).

: **Read Only Memory (*ROM*) (** : Certain instructions for the calculator are permanently stored in ROM and can be accessed but cannot be changed. [Data](#data) in ROM is not lost if batteries are removed.
...
: **Softcode** : Softcoded [data](#data) is information stored in an [array](#array) at the beginning of the program, and read from it later. It is usually slower to read than [hardcoded](#hardcode) data, but the advantage is that the same code can often be used for different data stored in the same way.
...
: **[Spaghetti Code](spaghetti-code.html)** : The unofficial name given to code that heavily relies on [Goto](goto.html)/[Lbl](lbl.html) for its structure and organization.

: **Sprite** : A small image which is moved around the screen using code and/or repeated multiple times on the screen.

: **Stack** : A [data structure](#data) much like a list but with limited access: only the top element at a given time can be accessed or changed. The [OS](#os) uses two stacks internally for running TI-Basic programs: one for evaluating expressions, and one for keeping track of Ends and program calls.

: **Statement** : A single line of a program containing a single instruction such as [ClrHome](clrhome.html), [Output(](output.html), [Stop](stop.html), etc. Usually synonymous with [command](#command).

: **[String](strings.html) (*Str*)** : A variable type that stores text as a series of symbols or [tokens](#token).

: **[Subprogram](subprogram.html)** : A program segment which can be used more than once during the execution of a program, such as a complex set of calculations or a print routine. You can make subprograms using the [prgm](prgm.html) command.
