...
While there is no size difference between the two, `DelVar` does have some problems that go along with using it. If used in a [`For`](for.html) loop to delete the counter variable or used to delete the variable and/or value in the [`IS>(`](is.html) or [`DS<(`](ds.html) commands before using them, it will cause an [ERR:UNDEFINED](errors.html#undefined) error.
...
- The [`End`](end.html) from an [`If`](if.html), [`Then`](then.html) block.
...
The speed of the `DelVar` command depends on the circumstance where it is used. When the variable already exists, `DelVar` is slower because it has to deallocate the variable from the RAM. `DelVar` is also significantly slower for zeroing real variables when compared to using [`→`](store.html) to set the variable to 0. The speed difference becomes apparent when the value is reset many times but is not a major factor if only used sparingly.

## Error Conditions

- **[ERR:SYNTAX](errors.html#syntax)** is thrown when trying to delete a system variable (e.g. `DelVar` Xmin) or a program, even though this is syntactically correct.
- **[ERR:UNDEFINED](errors.html#undefined)** is thrown if you delete the loop variable while inside the loop, or delete the variable used in [`IS>(`](is.html) or [`DS<(`](ds.html).
- **[ERR:ARCHIVED](errors.html#archived)** is thrown if you use `DelVar` on an archived variable.
