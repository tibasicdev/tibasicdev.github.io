...
Line(8,2,2,2
```
![http://tibasicdev.wdfiles.com/local—files/sk:more-shapes/square.gif](http://tibasicdev.wdfiles.com/local—files/sk:more-shapes/square.gif "")
...
The [Shade(](shade.html) function can be quite useful. This function darkens the window from one horizontal line to another, or from one function to another. When using Shade( , the order of the arguments does matter. This command will only shade points where the first argument is less than the second argument, and not those where the first argument is greater than the second argument; see the second picture below for an example of this.
...
Shade(-X+1,X/2
```
You may have noticed the use of a [Pause](pause.html) command on the Graph Screen. This is possible and is used quite commonly in some types of programs. It works the same as it does on the Home Screen, so to continue the program simply press Enter.
![http://tibasicdev.wdfiles.com/local—files/sk:more-shapes/Shade_picture1.png](http://tibasicdev.wdfiles.com/local—files/sk:more-shapes/Shade_picture1.png "")  ![http://tibasicdev.wdfiles.com/local—files/sk:more-shapes/Shade_picture2.png](http://tibasicdev.wdfiles.com/local—files/sk:more-shapes/Shade_picture2.png "")
...
That was simple! Next, we need to create a loop to let the user roll the dice as long as they want and stop when they hit a key. We'll use the [getkey](getkey.html) and the [While](while.html) command. Again, try and create your own code as it will help you learn the routines better!
<details style="display: inline;"><summary>show</summary>
```
0→K
While not(K
...
|**[<< Graph Format Settings](sk:graph-settings.html)**|**[Table of Contents](starter-kit.html)**|**[Text and Text Sprites >>](sk:text.html)**|
| --- | --- | --- |
