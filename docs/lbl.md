...
Although the Lbl/Goto loop structure may seem like a good alternative to loops, it should be avoided whenever possible, which is especially important when you are first [planning](plan.html) a program. This is because it has several serious drawbacks associated with it:
...
- Using a Goto to exit any block of code requiring an End command causes a [memory leak](memory-leaks.html) — around 40 bytes of memory will be rendered useless each time you do it until the program finishes running, which will also slow down your program down.
...
Labels are also used with the [Menu(](menu.html) command. The same considerations apply as with Goto, except that (unless you write a [custom menu](custommenus.html) routine) there's no simple alternative to using labels with Menu(.

## Error Conditions

- **[ERR:INVALID](errors.html#invalid)** is thrown if this statement is used outside a program.
