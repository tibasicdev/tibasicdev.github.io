...
### Menu Location
While editing a program, press:<br># PRGM to enter the PRGM menu.<br># 0 to choose Goto, or use arrows.<br># 9 to choose Lbl, or use arrows.
# The Goto Command

The `Goto` command is used together with the [`Lbl`](lbl.html) command to jump (or branch) to another place in a program. When the calculator executes a `Goto` command, it stores the label name in memory, and then searches from the beginning of the program for the `Lbl` command with the supplied name. If it finds it, it continues running the program from that point; otherwise, if the label does not exist, it throws an [ERR: LABEL](errors.html#label) error.
...
You can position a `Lbl` command one or more lines before a `Goto` command to create a kind of loop structure. However, you have to provide the break-out code, since it isn't built-in. An [`If`](if.html) conditional is easiest, but if there is no code that ends the branching, then program execution will continue indefinitely, until you manually exit it (by pressing the ON key).
...
Although the `Goto` command may seem like a good alternative to loops, it should be avoided whenever possible, which is especially important when you are first [planning](plan.html) a program. This is because it has several serious drawbacks associated with it:
...
- In most cases, [`If`](if.html), [`For(`](for.html), [`While`](while.html), or [`Repeat`](repeat.html) can be used instead, saving space and improving speed.
- Using a `Goto` to exit any block of code requiring an `End` command causes a [memory leak](memory-leaks.html), which will not be usable until the program finishes running or executes a [`Return`](return.html) command, and which will slow down your program down. See below for ways to fix this.
...
If your program requires [cleanup](cleanup.html) after it finishes, but it can exit from several different places, use `Goto` and place a `Lbl` at that point. This saves memory over repeating the cleanup code every time you exit. The usual considerations about `Goto` don't apply here: since you're exiting the program, all memory leaks will be gone anyway, and speed isn't much of an issue for something that only gets done once.
...
Another possible use for `Goto` is in [program protection](protection.html) to break a program with an error without letting the user see where it happened. If the label that you want to `Goto` doesn't exist, you'll get a [ERR: LABEL error](errors.html#label), which doesn't provide a 2:Goto option. So, all you have to do is `Goto` a label that you know doesn't exist.
...
- **[ERR:LABEL](errors.html#label)** is thrown if the corresponding label doesn't exist.
...
- [Program Protection](protection.html)
