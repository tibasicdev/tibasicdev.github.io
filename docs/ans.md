...
- [Store](store.html) a value to a variable, such as `1→X`

- Place an expression or string on a line by itself, such as `1+2` or `"Hello"`

- Use the optional argument of the [`Pause`](pause.html) command such as `Pause X`. `Ans` will be updated to the new value.
...
- Storing to an [equation variable](system-variables.html#equation) such as `"X+1"→Y₁`

- Using [`DelVar`](delvar.html) to delete a variable (i.e. set its value to zero, if it's a real variable)

- [`For(`](for.html) loops

- Changing the value with [`IS>(`](is-.html) or [`DS<(`](ds-.html)
...
The [`augment`](augment.html)() function can also be used with `Ans` to add additional items to your list, for example:
