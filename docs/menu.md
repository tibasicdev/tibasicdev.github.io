...
When the Menu( command is encountered during a program, the menu screen is displayed with the specified menu title in white-on-black text on the top line and each menu item listed below on its own line, the pause indicator turns on, and execution [pauses](pause.html) until the user selects a menu item. There is a cursor that the user can move up and down the menu to select a menu item.
...
Because the Menu( command displays the menu screen instead of clearing the home screen, you do not need to put the [ClrHome](clrhome.html) command before it.
...
- **[ERR:LABEL](errors.html#label)** is thrown when an option is chosen whose label doesn't exist.
