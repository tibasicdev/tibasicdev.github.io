...
With our number stored in X, we loop through each of the digits of the number starting from the right using the [seq(](seq-list.html) command. We get the respective digit by raising the number to the respective power of 10 and then multiplying the result by 10. The digits are returned separately in a list, and then we take the sum of them using the [sum(](sum.html) command.
...
- [Number Concatenation](number-concatenation.html)
- [Number Subset](number-subset.html)
