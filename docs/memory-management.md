# Memory Management
The TI-83 and TI-84 series of [calculators](thecalcs.html) feature a considerable amount of memory available for storing [variables](variables.html), [programs](sk:first-program.html), [groups](grouping.html), and even [assembly applications](assembly.html) (the last two are only available on the TI-83+/TI-84+/SE calculators). However, as you start putting those things on the calculator, the memory slowly gets used up, and if you don't do anything, the calculator will eventually run out of memory — giving you the dreaded [ERR:MEMORY](errors.html#memory) error.
...
- **[DelVar](delvar.html)** — DelVar is useful for deleting variables, which is the most obvious way to manage memory, and it is what most people are interested in. The DelVar command can delete any variable that you want, with exception to specific elements of a matrix or string or [system variables](system-variables.html).
- **[ClrList](clrlist.html)/[ClrAllLists](clralllists.html)** — Similar to DelVar, ClrList and ClrAllLists only work with [lists](lists.html) and they set the dimensions of one list or all lists to zero respectively. This essentially causes the list(s) to be deleted, since you can't do anything with a zero element list.
- **[Clear Entries](clear-entries.html)** — When executing programs or doing math on the calculator's home screen, the calculator keeps a history of these entries (you can cycle through them by pressing 2nd ENTER). If you do a lot of stuff on the home screen, the entries history can become rather large.
- **[Archive](archive.html)/[UnArchive](unarchive.html)** — When using variables and programs, you need to move them from archive to RAM; and when you are done using them, you move them back to archive. While you can archive programs on the home screen, that is not possible from inside a program (although you can use an [assembly library](asmlibs.html) to do that).
- **[GarbageCollect](garbagecollect.html)** — As you archive and unarchive variables and programs, the calculator keeps storing things until it eventually needs to clean the archive memory. Rather than simply leaving this until the calculator finally forces you to do it, which takes a fair amount of time, you can run the GarbageCollect command periodically.

Since the TI-83 calculator only has RAM memory, it does not have the Archive, UnArchive, and GarbageCollect commands. If you plan on [porting](portability.html) a program to the TI-83, you shouldn't use any of these commands, since they will cause the program to be corrupted.  In the case of [lists](lists.html), however you can use the [SetUpEditor](setupeditor.html) command instead of UnArchive to get around this problem.
Also note that trying to use DelVar or ClrList with an archived variable does not work, and will actually return an [ERR:ARCHIVED](errors.html#archived) error.
