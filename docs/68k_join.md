# Join the Wiki
So, you want to become a member of this site? Tell us why and apply now! Your application will be quickly reviewed, and a site administrator will accept your membership.



You can also join by entering a password. We have placed the password on the [contribute](contribute.html) page, so you need to read through that page to find it.
